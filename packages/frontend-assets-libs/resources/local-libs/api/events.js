...
mw.trigger = function (eventName, paramsArray) {
    return mw
        .$([window, mw._on._eventsRegister])
        .trigger(eventName, paramsArray);
};
...
mw._on = {
        ...
        if (trigger === true) {
            ...
            if (index !== -1) {
                ...
                if (
                    ...
                    mw.on._hashparam_funcs[index] !== undefined
                ) {
    ...
    hashParamEventInit: function () {
        ...
        if (hash === "" || hash === "#" || hash === "#?") {
            ...
            for (var x in params) {
                if (
                    params[x] !== mw.on._hashrec[x] ||
                    typeof mw.on._hashrec[x] === "undefined"
                ) {
    ...
    DOMChange: function (element, callback, attr, a) {
        ...
        if (typeof MutationObserver === "function") {
            ...
            element.addEventListener(
                ...
                function (e) {
                    if (
                        /*mw.tools.hasClass(e.target, 'element') || */ mw.tools.hasClass(
                            e.target,
                            "module"
                        ) ||
                        mw.tools.hasParentsWithClass(e.target, "module")
                    ) {
                        return false;
                    }
                    if (!mw.on.DOMChangePause) {
    ...
    scrollBarOnBottom: function (obj, distance, callback) {
        ...
        mw.$(obj).scroll(function (e) {
            var h =
                obj === window ? document.body.scrollHeight : obj.scrollHeight;
    ...
    userIteractionInit: function () {
        var max = 378;
        mw.$(document).on(
            "mousemove touchstart click keydown resize ajaxStop",
            function () {
...
$(window).on("hashchange load", function (event) {
    ...
    if (event.type === "hashchange") {
        ...
        var changes = mw.url.whichHashParamsHasBeenRemoved(
                mw.hashHistory[size - 1],
                mw.hashHistory[size - 2]
            ),
            l = changes.length,
            i = 0;
        if (l > 0) {
