        ...
        return {
            normalizeNesting(cmn) {
                if (!cmn) {
                    ...
                    cmn = this.scope.api.elementNode(
                ...
                const nodes = cmn.querySelectorAll(
                    "span,strong,b,i,em,font,small,big,u,s,strike"
                );
                ...
                nodes.forEach((node) => {
                    normalizeStyles.forEach((st) => {
                        if (node.style[st]) {
                            if (
                                getComputedStyle(node.parentNode)[st] ===
                                node.style[st]
                            ) {
                                node.style[st] = "";
                            }
                    ...
                    if (
                        ...
                        style === node.lastChild.getAttribute("style") &&
                        !node.id &&
                        !node.lastChild.id
                    ) {
                        toUnwrap.push(node);
                    } else if (
                        node.nodeName === node.parentNode.nodeName &&
                        node.attributes.length === 0
                    ) {
                        toUnwrap.push(node);
                    }
            ...
            normalizeStyles: function (target) {
                ...
                if (target) {
                    ...
                    var all =
                        target.parentNode.querySelectorAll('*[style*="var"]');
            ...
            textAlign: function (value) {
                ...
                if (api.isCrossBlockSelection()) {
                    ...
                    api.action(parent, function () {
                        if (
                            scope.settings.editMode === "liveedit" &&
                            mw.top().app.cssEditor
                        ) {
            ...
            format: function (value) {
                ...
                const edit =
                    mw.tools.firstParentOrCurrentWithClass(el, "edit") ||
                    scope.$editArea[0];
                ...
                var textNodeHandle = function () {
                    ...
                    var getFocusedNeighbours =
                        api.getFocusedNeighbours(focusedNode);
                ...
                var isTxtLike2 =
                    focusedNode.nodeType === 3 &&
                    !_availableTags.find(
                        (obj) => obj.value === nn && obj.value !== "div"
                    );

                var formattagsArray = _availableTags
                    .filter((obj) => obj.value !== "div")
                    .map((o) => o.value);
                ...
                if (
                    ...
                    focusedNode.querySelector(formattags)
                ) {
                    return;
                }
                ...
                while (focusedNodeBlock) {
                    if (
                        formattagsArray.indexOf(
                            focusedNodeBlock.nodeName.toLowerCase()
                        ) !== -1
                    ) {
            ...
            cleanStyle: function (propety) {
                ...
                function perNode(node) {
                    ...
                    if (mw.top().app.cssEditor) {
                        mw.top().app.cssEditor.removeSheetRuleProperty(
                            sel,
                            propety
                        );
                    }
            ...
            isCrossBlockSelection: function () {
                ...
                let startNode = mw.tools.firstBlockLevel(
                    scope.api.elementNode(sel.anchorNode)
                );
                let endNode = mw.tools.firstBlockLevel(
                    scope.api.elementNode(sel.focusNode)
                );
            ...
            getSelectionChildren: function () {
                ...
                const commonAncestorContainer = scope.api.elementNode(
            ...
            getSelectionChildNodes: function () {
                ...
                const nodes = Array.from(commonAncestorContainer.childNodes)
                    .filter((node) => range.intersectsNode(node))
                    .filter(
                        (node) =>
                            node.nodeType === 1 ||
            ...
            isPlainText: function (sel) {
                ...
                const isNativePlainText =
                    mw.tools.firstMatchesOnNodeOrParent(
                        anchorNode,
            ...
            setCursorAt: function (target, toStart = true) {
                ...
                range = (
                    sel.focusNode ? sel.focusNode.ownerDocument : document
                ).createRange();
            ...
            cleanWord: function (content) {
                var wrapListRoots = function () {
                    var all =
                            scope.$editArea.querySelectorAll("li[data-level]"),
                        i = 0,
                        has = false;
                    for (; i < all.length; i++) {
                        ...
                        if (parent !== "OL" && parent !== "UL") {
                            ...
                            var el = scope.actionWindow.document.createElement(
                                all[i].getAttribute("data-type") === "ul"
                                    ? "ul"
                                    : "ol"
                            );
                ...
                var buildWordList = function (lists, count) {
                    ...
                    lists.each(function () {
                        var num = this.textContent.trim().split(".")[0],
                            check = parseInt(num, 10);
                        var curr = mw.$(this);
                        ...
                        if (!this.__done) {
                            ...
                            if (!isNaN(level) && level > 1) {
                                ...
                                if (!!prev && prev.nodeName === "LI") {
                                    ...
                                    var wrap =
                                        scope.actionWindow.document.createElement(
                                            type === "ul" ? "ul" : "ol"
                                        );
                    ...
                    mw.$("ul[data-level!='1'], ol[data-level!='1']").each(
                        function () {
                            var level = parseInt($(this).attr("data-level"));
                            if (!!this.previousElementSibling) {
                                var plevel = parseInt(
                                    $(this.previousElementSibling).attr(
                                        "data-level"
                                    )
                                );
                                if (level > plevel) {
                ...
                var cleanWord = function (html) {
                    ...
                    html = html.replace(
                        /<table([^>]*)>/gi,
                        '<table cellspacing="0" cellpadding="0" border="1" style="width:100%;" width="100%" class="element">'
                    );
                ...
                var cleanTables = function (root) {
                    var toRemove =
                            "tbody > *:not(tr), thead > *:not(tr), tr > *:not(td)",
                    ...
                    for (; i < l; i++) {
                        ...
                        for (; i < l; i++) {
                            ...
                            if (
                                ...
                                var posibles =
                                    "thead tfoot tr tbody col colgroup";
            ...
            action: function (targetParent, func, recordTimeout) {
                ...
                scope.api._actionTimeout = setTimeout(
                    function () {
                        const edit =
                            mw.tools.firstParentOrCurrentWithClass(
                                targetParent,
            ...
            elementNode: function (c) {
                ...
                try {
                    /* Firefox returns wrong target (div) when you click on a spin-button */
            ...
            getFocusedNeighbours: function (focused) {
                ...
                while (next) {
                    if (
                        next.nodeType === 3 ||
                        tags.indexOf(next.nodeName.toLowerCase()) !== -1
                    ) {
                        res.push(next);
                ...
                while (prev) {
                    if (
                        prev.nodeType === 3 ||
                        tags.indexOf(prev.nodeName.toLowerCase()) !== -1
                    ) {
                        res.unshift(prev);
            ...
            cssApplier: function (css) {
                ...
                var all = scope.actionWindow.document.querySelectorAll(
                        "." + clstemp
                    ),
                    l = all.length,
                    i = 0;
            ...
            isSafeMode: function (el) {
                ...
                var hasSafe = mw.tools.hasAnyOfClassesOnNodeOrParent(el, [
                    "safe-mode",
                ]);
                var regInsafe = mw.tools.parentsOrCurrentOrderMatchOrNone(el, [
                    "regular-mode",
                    "safe-mode",
                ]);
            ...
            _execCommandCustom: {
                removeFormat: function (cmd, def, val) {
                    ...
                    var all = common.querySelectorAll("*"),
                        l = all.length,
                        i = 0;
                    for (; i < l; i++) {
                        var el = all[i];
            ...
            cleanNesting: function (target) {
                ...
                var lit = target.querySelectorAll(
                    "ul > *:not(li), ol > *:not(li)"
                );
                if (lit.length) {
            ...
            afterExecCommand: function (parent) {
                ...
                Array.prototype.slice
                    .call(parent.querySelectorAll("[style],[id]"))
                    .forEach((node) => {
                        if (!!node.getAttribute("style")) {
                            ...
                            keys.forEach((key) => {
                                if (node.style[key]) {
                                    var toRemove =
                                        node.style[key].includes("var(") ||
                                        keysToRemove.indexOf(key) !== -1 ||
                                        keysValuesToRemove.find(
                                            (a) => a[key] === node.style[key]
                                        );
                                    if (toRemove) {
                                        node.style[key] = "";
                                    }
                        ...
                        if (node.id) {
                            var others = parent.querySelectorAll(
                                '[id="' + node.id + '"]'
                            );
                            if (others.length > 1) {
                                others.forEach((n) => {
            ...
            execCommandSimple: function (cmd, def, val) {
                if (window.tinyMCE && window.tinyMCE.activeEditor) {
                    var camelcasecmd = cmd.replace(/-([a-z])/g, function (g) {
            ...
            execCommand: function (cmd, def, val, recordTimeout) {
                scope.actionWindow.document.execCommand(
                    "styleWithCss",
                    "false",
                    false
                );
                ...
                try {
                    // 0x80004005
                    if (
                        scope.actionWindow.document.queryCommandSupported(
                            cmd
                        ) &&
                        scope.api.isSelectionEditable()
                    ) {
                        def = def || false;
                        val = val || false;
                        if (sel.rangeCount > 0) {
                            var node = scope.api.elementNode(sel.focusNode);
                            var parent =
                                mw.tools.firstBlockLevel(node).parentNode
                                    .parentNode;
                            ...
                            scope.api.action(
                                ...
                                function () {
                                    function _execCommand(cmd, def, val) {
                                        ...
                                        var current = parent.querySelector(
                ...
                if (scope.api.isSelectionEditable()) {
                    ...
                    scope.api.action(parent.parentNode, function () {
                        if (scope.api.isCrossBlockSelection()) {
                            const childNodes =
                                scope.api.getSelectionChildNodes();
                            childNodes.forEach((node) => {
                                if (scope.editArea === node) {
                                } else {
            ...
            fontSize: function (size) {
                ...
                var range = sel.getRangeAt(0),
                    common = scope.api.elementNode(
                        range.commonAncestorContainer
                    );
                var nodrop_state =
                    mw.tools.parentsOrCurrentOrderMatchOrOnlyFirstOrNone(
                        common,
                        ["allow-drop", "nodrop"]
                    );
            ...
            restoreSelection: function () {
                if (scope.api.savedSelection) {
                    scope.api.savedSelection.element.attr(
                        "contenteditable",
                        "true"
                    );
                    scope.api.savedSelection.element.focus();
                    scope.api.savedSelection.selection.removeAllRanges();
                    scope.api.savedSelection.selection.addRange(
                        scope.api.savedSelection.range
                    );
                }
            },
            ...
            cleanHTML: function (html) {
                ...
                var elements = Array.prototype.slice.call(
                    this._cleaner.querySelectorAll("iframe,script,noscript")
                );
                ...
                Array.prototype.slice
                    .call(this._cleaner.querySelectorAll("[style]"))
                    .forEach((node) => {
                        const keys = Array.from(node.style);
                        keys.forEach((key) => {
            ...
            insertImage: function (url) {
                ...
                var img =
                    '<div class="element"><img style="max-width: 100%;" id="' +
                    id +
                    '" alt="' +
                    url +
                    '" src="' +
            ...
            link: function (result) {
                var sel = scope.getSelection();
                var common = scope.api.elementNode(
                    sel.getRangeAt(0).commonAncestorContainer
                );
                var el = scope.api.elementNode(sel.focusNode);
                var elLink = el.closest("a");

                if (elLink) {
                    scope.api.action(
                        mw.tools.firstBlockLevel(elLink),
                        function () {
                    ...
                    scope.api.execCommand("createLink", false, result.url);

                    if (result.target) {
                        setTimeout(() => {
                            common
                                .querySelectorAll(`a[href="${result.url}"]`)
            ...
            unlink: function () {
                ...
                if (!sel.isCollapsed) {
                    ...
                    var link = mw.tools.firstParentOrCurrentWithTag(
                        this.elementNode(sel.focusNode),
                        "a"
                    );
