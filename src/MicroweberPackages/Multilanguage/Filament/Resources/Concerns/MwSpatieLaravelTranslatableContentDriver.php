...
class MwSpatieLaravelTranslatableContentDriver implements TranslatableContentDriver
{
...
//
        ...
        /*  public function applySearchConstraintToQuery(Builder $query, string $column, string $search, string $whereClause, ?bool $isCaseInsensitivityForced = null): Builder
    {
        $databaseConnection = $query->getConnection();
         // Force case insensitive if not specified
        $isCaseInsensitivityForced = $isCaseInsensitivityForced ?? true;

        // Clean and prepare search terms
        $searchTerms = array_filter(explode(' ', trim($search)));

        return $query->{$whereClause}(function ($subQuery) use ($column, $searchTerms, $whereClause, $isCaseInsensitivityForced, $databaseConnection) {
            foreach ($searchTerms as $term) {
                $searchPattern = (string) str($term)->wrap('%', '%')->toString();

                // Use orWhere for broader matches
                $subQuery->orWhere(
        ...
        return $query->{$whereClause}(
            generate_search_column_expression($column, $isCaseInsensitivityForced, $databaseConnection),
            'like',
            (string) str($search)->wrap('%', '%')->toString()
        );
    }
}
