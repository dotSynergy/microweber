...
<div class="well">
    <?php if (count($payment_options) > 0): ?>
        <h2 style="margin-top:0 " class="edit nodrop mt-2" field="checkout_payment_information_title" rel="global" rel_id="<?php print $params['id'] ?>">
            ...
            foreach ($payment_options as $payment_option) : $count++; ?>
                <li>
                    <label class="mw-ui-check tip" data-tipposition="top-left" data-tip="<?php print  $payment_option['name']; ?>">
                        ...
                        <input type="radio"
                               <?php if ($count == 1): ?> checked="checked" <?php endif; ?>
                               <?php if (!empty($is_free_order)): ?> disabled="disabled" <?php endif; ?>
                               value="<?php print  $payment_option['gw_file']; ?>" name="payment_gw"/><span></span>
