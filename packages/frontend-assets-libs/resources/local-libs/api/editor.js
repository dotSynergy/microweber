...
var MWEditor = function (options) {
    (function () {
        ...
        var RtlDetect = (self = {
            ...
            _parseLocale: function (strLocale) {
                ...
                parsedLocale = { lang: lang, countryCode: countryCode };
                return parsedLocale;
            },
    ...
    var defaults = {
        ...
        interactionControls: ["image", "linkTooltip", "tableManager"],
        language: "en",
        rootPath: mw.settings.includes_url + "api/editor",
    ...
    this.settingsExtend = function (options = {}, trigger = true) {
        this.settings = mw.object.extend(
            {},
    ...
    if (!this.settings.direction) {
        if (this.settings.inputLanguage) {
            this.settings.direction = MWEditor.rtlDetect.getLangDir(
    ...
    if (typeof this.settings.controls === "string") {
        this.settings.controls =
            EditorPredefinedControls[this.settings.controls] ||
            EditorPredefinedControls.default;
    }
    ...
    if (!!this.settings.smallEditor) {
        if (this.settings.smallEditor === true) {
            this.settings.smallEditor =
                EditorPredefinedControls.smallEditorDefault;
        } else if (typeof this.settings.smallEditor === "string") {
            this.settings.smallEditor =
                EditorPredefinedControls[this.settings.smallEditor] ||
                EditorPredefinedControls.smallEditorDefault;
        }
    }
    ...
    this.settings.isTextArea =
        this.settings.selectorNode.nodeName &&
        this.settings.selectorNode.nodeName === "TEXTAREA";
    ...
    this.lang = function (key) {
        if (
            MWEditor.i18n[this.settings.language] &&
            MWEditor.i18n[this.settings.language][key]
        ) {
    ...
    var notEditableSelectors = scope.settings.notEditableClasses
        ? scope.settings.notEditableClasses
              .map((c) => `.${c}:not([contenteditable="false"])`)
              .join(",")
        : null;
    var instance = this;

    var actionLikeEvents = [
        "click",
        "execCommand",
        "keydown",
        "action",
    ...
    function _observe(e) {
        ...
        if (localTarget) {
            if (localTarget.parentNode) {
                ...
                while (parentobserve) {
                    if (
                        parentobserve.classList &&
                        parentobserve.classList.contains("no-typing")
                    ) {
            ...
            if (localTarget.querySelectorAll) {
                ...
                if (childNodes) {
                    childNodes.forEach(
                        (node) => (node.contentEditable = false)
                    );
                }
            }
        }
        ...
        if (
            e.type === "keydown" &&
            e.key === "Enter" &&
            !mw.tools.hasAnyOfClassesOnNodeOrParent(
                localTarget,
                _smallEditorExceptionClasses
            )
        ) {
        ...
        if (eventIsActionLike) {
            ...
            if (!shouldCloseSelects && e.target) {
                var smallEditor =
                    !!scope.smallEditor && scope.smallEditor.get(0);
                if (smallEditor) {
        ...
        if (eventIsActionLike || time - scope._interactionTime > max) {
            ...
            var target =
                scope.api.elementNode(range.startContainer) || scope.area;
            ...
            if (
                scope.editArea.contains(scope.selection.anchorNode) &&
                scope.editArea.contains(scope.selection.focusNode)
            ) {
                rangeInEditor = true;
            }
            ...
            var iterData = {
                ...
                isImage:
                    localTarget.nodeName === "IMG" || target.nodeName === "IMG",
                css: css.get,
                cssNative: css.css,
                event: event,
                api: api,
            ...
            if (notEditableSelectors) {
                const all =
                    scope.editArea.querySelectorAll(notEditableSelectors);
                let i = 0,
                    l = all.length;
                for (; i < l; i++) {
    ...
    this.initInteraction = function () {
        ...
        scope.state.on("redo", function () {
            ...
            var target = active
                ? active.target
                : scope.getSelection().focusNode;
            setTimeout(function () {
                _observe();
            }, 123);
        });
        scope.$editArea.on(
            "touchstart touchend click keydown execCommand mousemove touchmove scroll mouseup",
            _observe
        );
    ...
    this.preventEvents = function () {
        ...
        if (this.area && this._preventEvents.indexOf(this.area.node) === -1) {
            ...
            scope.$iframeArea &&
            this._preventEvents.indexOf(scope.$iframeArea[0]) === -1
        ) {
    ...
    var areaSelectionMemo = function () {
        scope.actionWindow.document.addEventListener(
            "selectionchange",
            function (e) {
                var sel = scope.getSelection();
                if (
                    scope.editArea.contains(sel.anchorNode) &&
                    scope.editArea.contains(sel.focusNode)
                ) {
    ...
    this.createFrame = function () {
        ...
        $(this.frame).on("load", function () {
            ...
            scope.$iframeArea = $(
                scope.settings.iframeAreaSelector,
    ...
    this.createWrapper = function () {
        ...
        this.wrapper.className =
            "mw-editor-wrapper mw-editor-" + this.settings.skin;
    };
    ...
    this.createArea = function () {
        ...
        this.area = mw.element({
            props: {
                className: "mw-editor-area",
                innerHTML: content,
                dir: scope.settings.direction,
                style: {
                    direction: scope.settings.direction,
                    textAlign:
                        scope.settings.direction === "ltr" ? "left" : "right",
                },
            },
        });
        setTimeout(() => {
    ...
    this.addControllerGroup = function (obj, row, bar) {
        ...
        setTimeout(function () {
            ...
            if (doc && !doc.body.__mwEditorGroupDownRegister) {
                ...
                doc.body.addEventListener("click", function (e) {
                    const parentTarget = mw.tools.firstParentOrCurrentWithClass(
                        e.target,
                    ...
                    if (e.target !== doc.body && !parentTarget) {
                        mw.element(
                            ".mw-bar-control-item-group.active",
                            doc.body
                        ).each(function () {
        ...
        var icon = MWEditor.core.button({
            ...
            props: {
                ...
                innerHTML:
                    '<span class="mw-editor-group-button-caret"><svg viewBox="0 0 24 24"><path fill="currentColor" d="M7,10L12,15L17,10H7Z" /></svg></span>',
            },
        });
        if (group.icon) {
            if (group.icon.includes("<")) {
                icon.prepend(
                    '<span class="mw-editor-group-button-icon">' +
                        group.icon +
                        "</span>"
                );
            } else {
                icon.prepend(
                    '<span class="' +
                        group.icon +
                        ' mw-editor-group-button-icon"></span>'
                );
            }
            ...
            if (scope.controllers[group.controller]) {
                var ctrl = new scope.controllers[group.controller](
                    scope,
                    scope.api,
                    scope
                );
        ...
        row =
            typeof row !== "undefined"
                ? row
                : this.settings.controls.length - 1;
    ...
    this.controlGroupManager = function () {
        var check = function () {
            var i = 0,
                l = scope._addControllerGroups.length;
            for (; i < l; i++) {
                ...
                if (
                    ...
                    item.el.$node[match.matches ? "addClass" : "removeClass"](
                        "mw-editor-control-group-media-matches"
                    );
                }
            }
        };
    ...
    function smallEditorPinned(state) {
        if (typeof state === "undefined") {
            ...
            if (scope.settings.id) {
                if (scope.storage) {
                    pinned = scope.storage.get(
                        scope.settings.id + "-small-editor-pinned"
                    );
                }
    ...
    var _afterPin = function () {
        setTimeout(function () {
            if (scope.lastRange) {
                scope.smallEditorInteract(
                    scope.getActualTarget(
                        scope.lastRange.commonAncestorContainer
                    )
                );
            }
        }, 178);
    ...
    this.setSmallEditorSkin = function (skin) {
        this.smallEditor.removeClass(
            "mw-small-editor-skin-" + this.settings.smallEditorSkin
        );
        this.settings.smallEditorSkin = skin;
        this.smallEditor.addClass(
            "mw-small-editor-skin-" + this.settings.smallEditorSkin
        );
    ...
    this.createSmallEditor = function () {
        ...
        this.smallEditor = mw.element({
            props: {
                className:
                    "mw-small-editor mw-small-editor-skin-" +
                    (this.settings.smallEditorSkin || this.settings.skin),
            },
        });
        ...
        scope.$editArea.on("click", function (e) {
            var target =
                e.target !== scope.actionWindow.document.body
                    ? scope.getActualTarget(e.target)
                    : scope.actionWindow.document.body;
    ...
    scope.getActualTarget = function (target) {
        return mw.tools.firstParentOrCurrentWithTag(
            scope.api.elementNode(target),
            [
                "div",
                "ul",
                "ol",
    ...
    this.positionSmallEditor = function (target) {
        ...
        ctop = Math.max(
            ctop,
            scope.settings.document.defaultView?.scrollY + 25
        );
    ...
    this.smallEditorInteract = function (target) {
        ...
        _smallEditorInteractTimeout = setTimeout(() => {
            ...
            if (
                ...
                scope.lastRange &&
                scope.lastRange.collapsed === false
            ) {
                target = scope.getActualTarget(
                    scope.lastRange.commonAncestorContainer
                );
            }
            if (
            ...
            if (
                ...
                ((target && target.isContentEditable) ||
                    mw.tools.hasAnyOfClassesOnNodeOrParent(target, [
                        "mw-small-editor",
                        "mw-editor",
                        "mw-tooltip",
                    ])) &&
                scope.api.isSelectionEditable() /* && !scope.selection.isCollapsed*/
            ) {
    ...
    this.createBar = function () {
        ...
        if (this.settings.controls.length === 0) {
            ...
            setTimeout(
                () =>
                    this.wrapper.parentNode.classList.add(
                        "mw-editor-area-wrapper-empty"
                    ),
                100
            );
        }
        ...
        $(this).on("ready", function () {
            ...
            if (scope.settings.editMode === "liveedit") {
                ...
                const set = function (e) {
                    ...
                    Array.from(
                        scope.settings.document.querySelectorAll(
                    ...
                    if (edit) {
                        const hasSafe = mw.tools.hasAnyOfClassesOnNodeOrParent(
                            e.target,
                            ["safe-mode"]
                        );
                        const regInsafe =
                            mw.tools.parentsOrCurrentOrderMatchOrNone(
                                e.target,
                                ["regular-mode", "safe-mode"]
                            );
                ...
                $(scope.settings.regions, scope.actionWindow.document).attr(
                    "contenteditable",
                    true
                );
            }
            ...
            scope.$editArea.on("paste input", async (event) => {
                ...
                if (
                    ...
                    if (!!off) {
                        ...
                        setTimeout(() => {
                            ...
                            var isSafeMode =
                                mw.tools.parentsOrCurrentOrderMatchOrOnlyFirst(
                                    scope.api.elementNode(
                                        scope.api.getSelection().focusNode
                                    ),
                            ...
                            const safeModeAllowedTags =
                                "a,b,br,del,em,i,font,s,samp,small,span,strong,sub,summary,sup,time,u,var,wbr";
                            ...
                            if (
                                ta.firstChild &&
                                ta.firstChild === ta.lastChild &&
                                ta.lastChild.nodeName === "DIV"
                            ) {
                            ...
                            ta.querySelectorAll("[class]").forEach((el) =>
                                el.removeAttribute("class")
                            );
                            ta.querySelectorAll("[style]").forEach((el) =>
                                el.removeAttribute("style")
                            );
                            ta.querySelectorAll("[id]").forEach((el) =>
                                el.removeAttribute("id")
                            );
                            //ta.querySelectorAll('.' + classesToremove.join(',.')).forEach(el => el.classList.remove(...classesToremove));

                            if (
                                plainTextNodes.includes(e.target.nodeName) ||
                                mw.tools.firstParentWithTag(e.target, titles)
                            ) {
                                content = ta.textContent;
                            } else if (isSafeMode) {
                                let all = ta.querySelectorAll(
                                    `*:not(${safeModeAllowedTags})`
                                );
                                while (all.length) {
                                    all.forEach((node) =>
                                        node.replaceWith(...node.childNodes)
                                    );
                                    all = ta.querySelectorAll(
                                        `*:not(${safeModeAllowedTags})`
                                    );
                                }
                                ...
                                ta.querySelectorAll("[style]").forEach(
                                    (node) => {
                            ...
                            if (!!content) {
                                ...
                                var range = scope.api
                                    .getSelection()
                                    .getRangeAt(0);
                                var doc = this.actionWindow.document
                                    .createRange()
                                    .createContextualFragment(content);
                                range.deleteContents();
                                range.insertNode(doc);
                                var txt =
                                    this.actionWindow.document.createTextNode(
                                        "\u200B"
                                    );
                                ...
                                if (
                                    mw.top &&
                                    typeof mw.top().app === "object" &&
                                    typeof mw.top().app.registerChangedState ===
                                        "function"
                                ) {
                                    mw.top().app.registerChangedState(
                                        txt.parentNode
                                    );
                                }
                            }
                    ...
                    setTimeout(() => {
                        var all = scope.api
                            .elementNode(scope.getSelection().focusNode)
                            .parentNode.querySelectorAll('*[style*="var"]');
                ...
                if (scope.actionWindow.mw && scope.actionWindow.mw.wysiwyg) {
                    scope.actionWindow.mw.wysiwyg.normalizeBase64Images(
                        this.parentNode,
                        function () {
                            scope.registerChange();
                        }
    ...
    this.liveEditMode = function () {
        this.liveedit = MWEditor.liveeditMode(
            this.actionWindow.document.body,
            scope
        );
    };
    ...
    this.__insertEditor = function () {
        if (this.settings.isTextArea) {
            ...
            mw.$(this.settings.selector).append(this.wrapper)[0].mwEditor =
                this;
        }
    };
    ...
    this.init = function () {
        ...
        this.settings.document.defaultView.addEventListener(
            "scroll",
            function () {
    ...
    mw.Editor = function (options) {
        ...
        if (options.selector) {
            if (typeof options.selector === "string") {
                options.selector = (options.document || document).querySelector(
                    options.selector
                );
            }
            if (
                options.selector &&
                options.selector.__MWEditor &&
                !options.forced
            ) {
