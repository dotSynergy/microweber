...
class LiveEditCanvasBase extends MicroweberBaseClass {
    ...
    findByKey(key, excludeSameIdentity = false) {
        ...
        for (let identity in curr) {
            if (
                curr[identity][key] &&
                curr[identity][key] &&
                (!excludeSameIdentity || identity !== this.getIdentity())
            ) {
    ...
    findByKeyValue(key, value, excludeSameIdentity = false) {
        ...
        for (let identity in curr) {
            if (
                curr[identity][key] &&
                this.urlAsValue(curr[identity][key]) === value &&
                (!excludeSameIdentity || identity !== this.getIdentity())
            ) {
                res.push({ ...curr[identity], identity });
            }
...
export class LiveEditCanvas extends LiveEditCanvasBase {
    ...
    isUrlOpened(url) {
        return (
            this.findByKeyValue("canvasURL", this.urlAsValue(url), false)
                .length > 1
        );
    }
    ...
    getLiveEditData() {
        const res = this.getWindow()?.mw?.liveEditIframeData;
        return !!res && res;
    }
    ...
    #documentDefaultStyles() {
        let style = this.getDocument().querySelector(
            "style#mw-le-document-default-styles"
        );
        if (!style) {
            style = document.createElement("style");
            style.id = "mw-le-document-default-styles";
            style.textContent = `
                .mw-has-live-live-edit--false {
                    .mw-bg-image-handles,
                    .mw-editor-link-tooltip,
            ...
            `;
            this.getDocument().body.appendChild(style);
        }
    }
    ...
    mount(target) {
        ...
        window.onbeforeunload = () => {
            if (
                liveEditIframe &&
                liveEditIframe.contentWindow &&
                liveEditIframe.contentWindow.mw &&
        ...
        liveEditIframe.addEventListener("load", (e) => {
            ...
            this.#documentDefaultStyles();

            frameWindow.document.documentElement?.classList.remove(
                "mw-has-live-live-edit--true",
                "mw-has-live-live-edit--false"
            );
            frameWindow.document.documentElement?.classList.add(
                `mw-has-live-live-edit--${!!this.getLiveEditData()}`
            );

            this.getDocument().documentElement.dataset.mwTheme = mw
                .top()
                .admin.theme.getTheme();
            ...
            if (
                ...
                liveEditIframe.contentWindow.document.body.classList.add(
                    "live-edit-frame-loaded"
                );
            }
            ...
            ["mousedown", "touchstart"].forEach((e) => {
                liveEditIframe.contentWindow.document.body.addEventListener(
                    e,
                    (event) => this.dispatch("canvasDocumentClickStart", event)
                );
            });
            ...
            liveEditIframe.contentWindow.document.body.addEventListener(
                ...
                (event) => {
                    if (
                        mw.app.liveEdit.handles.targetIsOrInsideHandle(
                            event.target
                        )
                    ) {
                        return;
            ...
            liveEditIframe.contentWindow.document.addEventListener(
                "keydown",
                (event) => {
                    if (
                        mw.app.liveEdit.handles.targetIsOrInsideHandle(
            ...
            this.dispatch("liveEditCanvasLoaded", {
                frame: liveEditIframe,
                frameWindow: liveEditIframe.contentWindow,
                frameDocument: liveEditIframe.contentWindow.document,
            });
