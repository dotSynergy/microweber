...
const config = {
    ...
    destination: '../../public/modules/captcha/js/',
    files: ['captcha-alpine.js'] // specific files to copy
};
...
// Copy all files from source directory (flat copy)
function copyFiles() {
    let successCount = 0;
    let totalCount = 0;
...
function watch() {
    ...
    fs.watch(config.source, { recursive: true }, (eventType, filename) => {
        if (filename && path.extname(filename) === '.js') {
            ...
            // If watching specific files, only rebuild if it's one of them
            if (config.files && config.files.length > 0) {
